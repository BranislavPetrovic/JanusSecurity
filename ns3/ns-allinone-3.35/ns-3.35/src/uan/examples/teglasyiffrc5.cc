...
UanExperiment::SetupPositions() {
    ...
    m_nodes.Get(1)->GetObject<MobilityModel> ()->SetPosition(Vector(4000, 0, 0));
}
...
UanExperiment::PrintReceivedPacket(Ptr<Socket> socket) {
    ...
    while (socket->GetRxAvailable() > 0) {
        ...
        double a = (double)Simulator::Now().GetMilliSeconds();
        double b = 0.0026934;
        double c = (double)l / 80;
        double d = 0.802678;
...
//        NS_LOG_UNCOND(to_string(timestamp));
        ...
        if (Mac8Address::IsMatchingType(srcAddress)) {
            NS_LOG_UNCOND("Received data from " << Mac8Address::ConvertFrom(srcAddress) << ": " << binPacket << ", timestamp: " << to_string(timestamp));
...
void UanExperiment::SendPacketTo1() {  
    ...
    timestamp = (double)Simulator::Now().GetMilliSeconds() + 0.002678 + (double)l / 80; // Now + RC5 encryption + Janus encoding
    NS_LOG_UNCOND("Sent packet to 1, timestamp: " << to_string(timestamp));
    //SendPacketTo0();

    //Simulator::Schedule(Hours(2), &UanExperiment::SendPacketTo1, this);
}

...
public:
  ...
  //void SendPackets(uint8_t* timestamp);
  ...
  while (socket->GetRxAvailable () > 0)
    {
      ...
      uint32_t timestamp;
      packet->CopyData (&timestamp, 1);

      if(Mac8Address::IsMatchingType (srcAddress))
        {
            NS_LOG_UNCOND ("Node: " << Mac8Address::ConvertFrom (srcAddress) << " | Timestamp: " << +timestamp << ' ' << 
                Simulator::Now().GetSeconds());
/*          NS_LOG_UNCOND ( "Time: " << Simulator::Now ().GetYears () << " years, " << Simulator::Now ().GetDays () << 
              " days, " << Simulator::Now ().GetHours () << "h " << 
              Simulator::Now().GetMinutes() << "m "  << Simulator::Now().GetSeconds() << " s" << " | Node: " <<
                          Mac8Address::ConvertFrom (srcAddress) << " | Timestamp: " << +timestamp);
*/
        }
    }
}
  ...
  while (node != m_nodes.End ())
    {
      ...
      uint32_t timestamp = Simulator::Now().GetSeconds();
      NS_LOG_UNCOND (+timestamp);
      Ptr<Packet> pkt = Create<Packet> (&timestamp, 1);

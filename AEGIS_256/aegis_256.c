...
static void AEGIS_initialize(AEGIS_state* S, uint8_t* key, uint8_t* IV) {
    //
    // The AEGIS-256 initialization function
    //
    // Input
    //
    // key - the key of length AES_KEYLEN bytes (32 bytes for AEGIS-256)
    // IV - the initialization vector of length AES_KEYLEN bytes (32 bytes for AEGIS-256)
    //
    // Output
    //
    // S - the initial state of AEGIS-256 (the registers s0 .. s5 of length AES_BLOCKLEN)
    //
    int i;
    uint8_t key_0[AES_BLOCKLEN];
    uint8_t key_1[AES_BLOCKLEN];
    uint8_t key_plus_IV_0[AES_BLOCKLEN];
    uint8_t key_plus_IV_1[AES_BLOCKLEN];
    for (i = 0; i < AES_BLOCKLEN; i++)
        key_0[i] = key[i];
    for (i = 0; i < AES_BLOCKLEN; i++)
        key_1[i] = key[i + AES_BLOCKLEN];
    for (i = 0; i < AES_BLOCKLEN; i++)
        key_plus_IV_0[i] = key[i]^IV[i];
    for (i = 0; i < AES_BLOCKLEN; i++)
        key_plus_IV_1[i] = key[i + AES_BLOCKLEN]^IV[i + AES_BLOCKLEN];
    for (i = 0; i < AES_BLOCKLEN; i++) {
        S->s0[i] = key_plus_IV_0[i];
        S->s1[i] = key_plus_IV_1[i];
        S->s2[i] = const1[i];
        S->s3[i] = const0[i];
        S->s4[i] = key[i]^const0[i];
        S->s5[i] = key[i + AES_BLOCKLEN]^const1[i];
    }
    for (i = 0; i < 16; i++) {
        if (i % 4 == 0)
            AEGIS_state_update(S, key_0);
        else if (i % 4 == 1)
            AEGIS_state_update(S, key_1);
        else if (i % 4 == 2)
            AEGIS_state_update(S, key_plus_IV_0);
        else
            AEGIS_state_update(S, key_plus_IV_1);
    }
    return;
}
...
static void AEGIS_state_update(AEGIS_state* S, uint8_t* m) {
    //
    // The AEGIS-256 state update function
    //
    // Input
    //
    ...
    // Input/output
    //
    // S - the state of AEGIS-256 (the registers s0 .. s5 of length AES_BLOCKLEN)
    //
    int i;
    AEGIS_state S1;
    ...
    // Running AES round function 6 times
    AES_round((state_t*) (S->s5), (state_t*) (S1.s0), s0_plus_m);
    ...
    AES_round((state_t*) (S->s4), (state_t*) (S1.s5), S->s5);
    ...
    memcpy(S->s5, S1.s5, AES_BLOCKLEN);
    return;
}

static void AEGIS_process_assoc_data(AEGIS_state* S, uint8_t* AD, uint64_t ADlen) {
    //
    // The AEGIS-256 function for processing associated data
    //
    // Input
    //
...
static void AEGIS_encrypt(AEGIS_state* S, uint8_t* C, uint8_t* P, uint64_t MSGlen) {
    //
    // The AEGIS-256 encryption function
    //
    // Input
    //
    ...
    for (i = 0; i < nblocks; i++) {
        ...
        for (j = 0; j < AES_BLOCKLEN; j++) {
            C_curblock[j] = P_curblock[j]^S->s1[j]^S->s4[j]^S->s5[j]^(S->s2[j] & S->s3[j]);
...
static void AEGIS_decrypt(AEGIS_state* S, uint8_t* C, uint8_t* P, uint64_t MSGlen) {
    //
    // The AEGIS-256 decryption function
    //
    // Input
    //
    // C - the ciphertext of length MSGlen bytes
    ...
    for (i = 0; i < nblocks; i++) {
        ...
        for (j = 0; j < AES_BLOCKLEN; j++) {
            P_curblock[j] = C_curblock[j]^S->s1[j]^S->s4[j]^S->s5[j]^(S->s2[j] & S->s3[j]);
            if (byte_count < MSGlen) {
...
static void AEGIS_finalize(AEGIS_state* S, uint8_t* Tprim, uint64_t adlen, uint64_t msglen) {
    //
    // The AEGIS-256 finalize function - produces the authentication tag Tprim
    //
    // Input
    //
    ...
    // Produce Tprim as an XOR of s0 .. s5
    for (i = 0; i < AES_BLOCKLEN; i++)
        Tprim[i] = S->s0[i]^S->s1[i]^S->s2[i]^S->s3[i]^S->s4[i]^S->s5[i];
    return;
}
...
void AEGIS_generation_encryption(uint8_t* key, uint8_t* IV, uint8_t* AD, uint64_t ADlen, uint8_t* P, uint8_t* C, uint64_t MSGlen, uint8_t* Tprim) {
    //
    // The generation-encryption function of AEGIS-256
    //
    // Input
    //
    // key - the key of length AES_KEYLEN bytes (32 bytes for AEGIS-256)
...
void AEGIS_decryption_verification(uint8_t* key, uint8_t* IV, uint8_t* AD, uint64_t ADlen, uint8_t* P, uint8_t* C, uint64_t MSGlen, uint8_t* Tprim) {
    //
    // The decryption-verification function of AEGIS-256
    //
    // Input
    //
    // key - the key of length AES_KEYLEN bytes (32 bytes for AEGIS-256)
